...
def add_polygon_rcnn_outputs(model, blob_in, dim):
...
def add_polygon_rcnn_losses(model):
    ...
    loss_bbox = model.net.SmoothL1Loss(
        [
            'polygon_pred', 'polygon_targets', 'polygon_inside_weights',
            'polygon_outside_weights'
        ],
        'loss_bbox',
        scale=model.GetLossScale()
    )
